...
% subject = {'1','2','3','4','5'};
% sData = strcat('propcut_freq_v2_',subject,'.mat');
...
for k = 1:nSubject
    ...
    p_d2_ft = fit( [fc(1:3) fc(5:7)]', [p_d2(k,1:3) p_d2(k,5:7)]', ft, opts );
    p_d2_ft.sigma = p_d2_ft.sigma;
    plot(fcGrid,p_d1_ft(fcGrid),'b','LineWidth',0.5)
    plot(fcGrid,p_d2_ft(fcGrid),'r','LineWidth',0.5)
    
    base_mu(k,:) =  [p_d1_ft.mu p_d2_ft.mu];
    base_sig(k,:) = [p_d1_ft.sigma p_d2_ft.sigma] * 1000;
...
d2_fd_no = fit( [fc(1:3) fc(5:7)]', [prob_d2(1:3) prob_d2(5:7)]', ft, opts );
d2_fd_no.sigma = d2_fd_no.sigma - 0.005;
...
% Far position.
subplot(231), hold on
plot(fcGrid,d1_fd_no(fcGrid),'b')
plot(fcGrid,d2_fd_no(fcGrid),'r')
h0 = errorbar([fc(1:3) fc(5:7)],[prob_d1(1:3) prob_d1(5:7)],[er_d1(1:3) er_d1(5:7)],'o'); set(h0,'Color','b','MarkerFaceColor','k','MarkerSize',1e-10,'LineWidth',2)
h00 = errorbar([fc(1:3) fc(5:7)],[prob_d2(1:3) prob_d2(5:7)],[er_d2(1:3) er_d2(5:7)],'o'); set(h00,'Color','r','MarkerFaceColor','k','MarkerSize',1e-10,'LineWidth',2)
...
for subj = 1:8
    rng(subj)
    dx = randn/10;
    plot(X(1,:)+dx, (mu_100(subj,:)),mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(2,:)+dx, (mu_200(subj,:)),mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(3,:)+dx, (mu_300(subj,:)),mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
end
...
for subj = 1:8
    rng(subj)
    dx = randn/10;
    plot(X(1,:)+dx, (sigma_100(subj,:)),mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(2,:)+dx, (sigma_200(subj,:)),mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(3,:)+dx, (sigma_300(subj,:)),mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
end
% Error bars
