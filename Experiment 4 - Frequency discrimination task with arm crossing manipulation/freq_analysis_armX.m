...
for subj = 1:8
    rng(subj)
    dx = randn/10;
    plot(X(:,1)+dx, [dpseunXed(subj,1) dpseunXed(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(:,2)+dx, [dpseXed(subj,1) dpseXed(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
end

% xlabel('Distractor (Hz)')
ylabel('\DeltaPSE (Hz)')
ylim([-60 70]); box off
set(gca,'YTick',[-60 0 70])
...
for subj = 1:8
    rng(subj)
    dx = randn/10;
    plot(X(:,1)+dx, [djndunXed(subj,1) djndunXed(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(:,2)+dx,[djndXed(subj,1) djndXed(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
end


ylabel('\DeltaJND (Hz)')
axis([-0.6 4.5 -21 70]); box off
set(gca,'YTick',[0 70])
