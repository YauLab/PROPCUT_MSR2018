...
for subj = 1:8
    rng(subj)
    dx = randn/10;
    plot(X(:,1)+dx, [dpsefar(subj,1) dpsefar(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(:,2)+dx,[dpsenear(subj,1) dpsenear(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
end
...
for subj = 1:8
    rng(subj)
    dx = randn/10;
    plot(X(:,1)+dx, [djndfar(subj,1) djndfar(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
    plot(X(:,2)+dx,[djndnear(subj,1) djndnear(subj,2)],mark{subj},'MarkerFaceColor','none','MarkerEdgeColor',[0.6 0.6 0.6],'MarkerSize',5)
end

% xlabel('Distractor'), 
ylabel('\DeltaJND (Hz)')
axis([-0.6 4.5 -7 45]); box off
set(gca,'YTick',[0 45])
