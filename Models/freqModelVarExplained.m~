...
for subj = 1:8
    ...
    ll_c_wo_jnd = [-4216.8      -4137.3      -4103.5      -4079.9      -4053.2      -4140.2      -4103.5      -4107.1];
    ll_jnd_plus_c = [-4215.7      -4112.8      -4081.6      -4061.3      -4032.6      -4119.8      -4079.2      -4084.6];
...
% Plots
subplot(211)
X = smartbar(meanrs,1,0.2,{'S1','S2','S3','S4','S5','S6','S7','S8','avg','CV'});
...
%% Plots to address reviewers' questions
clc; clf;
...
subplot(221)
pse = mean( reshape(x-out.pseD1',9,8), 2);
pse = reshape(pse,3,3);temp = pse(:,1);
pse(:,1) = pse(:,3); pse(:,3) = temp;

sem = std( reshape(x-out.pseD1',9,8), [], 2 );
sem = reshape(sem,3,3);temp = sem(:,1);
sem(:,1) = sem(:,3); sem(:,3) = temp;

X = smartbar(pse,0.9,0.4);
errorbar(X(:), pse(:), sem(:),'.k')
axis([-0.8 9.5 -60 80]); box off
set(gca,'TickDir','out')
...
subplot(222)
jnd = mean( reshape(y-out.jndD1',9,8), 2);
jnd = reshape(jnd,3,3);temp = jnd(:,1);
jnd(:,1) = jnd(:,3); jnd(:,3) = temp;

sem = std( reshape(y-out.jndD1',9,8), [], 2 );
sem = reshape(sem,3,3);temp = sem(:,1);
sem(:,1) = sem(:,3); sem(:,3) = temp;


X = smartbar(jnd,0.9,0.4);
errorbar(X(:), jnd(:), sem(:),'.k')
axis([-0.8 9.5 0 55]); box off
set(gca,'TickDir','out')
set(gca,'YTick',[0 30 55])
